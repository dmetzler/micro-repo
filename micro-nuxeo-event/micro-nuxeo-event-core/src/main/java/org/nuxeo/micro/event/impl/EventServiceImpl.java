...
import org.nuxeo.micro.event.EventBundleImpl;
...
import org.nuxeo.micro.event.EventListenerList;
import org.nuxeo.micro.event.EventService;
import org.nuxeo.micro.event.EventServiceAdmin;
import org.nuxeo.micro.event.EventStats;
...
public class EventServiceImpl implements EventService, EventServiceAdmin, Synchronization {
    ...
    public void shutdown(long timeoutMillis) throws InterruptedException {
        ...
        Set<AsyncWaitHook> notTerminated = asyncWaitHooks.stream().filter(hook -> !hook.shutdown())
    ...
    public void waitForAsyncCompletion(long timeout) {
        Set<AsyncWaitHook> notCompleted = asyncWaitHooks.stream().filter(hook -> !hook.waitForAsyncCompletion())
                .collect(Collectors.toSet());
    ...
    public void fireEvent(Event event) {
        ...
        // XXX Find add some stats.
        EventStats stats = null; // Framework.getService(EventStats.class);
    ...
    public void fireEventBundle(EventBundle event) {
        List<EventListenerDescriptor> postCommitSync = listenerDescriptors
                .getEnabledSyncPostCommitListenersDescriptors();
        List<EventListenerDescriptor> postCommitAsync = listenerDescriptors
