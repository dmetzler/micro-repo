...
public class LocalSession extends AbstractSession implements CloseableCoreSession, Synchronization {
     ...
     */
    private final Set<SessionInfo> allSessions = Collections.newSetFromMap(
            new ConcurrentHashMap<SessionInfo, Boolean>());

    public LocalSession(Repository repository, NuxeoPrincipal principal, CoreSessionServiceImpl coreSessionService) {

        super(coreSessionService, repository);
        ...
        sessionId = newSessionId(repository.getName(), principal);
    ...
    @Override
    public void close() {
        CoreInstance.closeCoreSession(this, css); // calls back destroy()
    }
