...
public abstract class UnrestrictedSessionRunner {
    ...
    /**
     * Constructs a {@link UnrestrictedSessionRunner} given a repository name and an
    ...
    /**
     * Calls the {@link #run()} method with an unrestricted {@link #session}. During
     * this call, {@link #isUnrestricted} is set to {@code true}.
     */
    public void runUnrestricted() {
        ...
        try {
            ...
            try (CloseableCoreSession closeableCoreSession = ((AbstractSession) session).upgradeAsSystem()) {
     ...
     * <p>
     * It can also be called directly in which case the {@link #session} available
     * will be the one passed to {@code #UnrestrictedSessionRunner(CoreSession)}.
     */
