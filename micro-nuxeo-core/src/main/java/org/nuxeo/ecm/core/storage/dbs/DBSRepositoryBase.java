...
public abstract class DBSRepositoryBase implements DBSRepository {
    ...
    private static final Logger log = LoggerFactory.getLogger(DBSRepositoryBase.class);
    ...
    protected static abstract class BlobFinder {
        ...
        public BlobFinder(SchemaManager sm) {
            this.sm = sm;
        }
        ...
        public void visit() {
            // document types
            for (DocumentType docType : sm.getDocumentTypes()) {
                visitSchemas(docType.getSchemas());
            }
            // mixins
            for (CompositeType type : sm.getFacets()) {
