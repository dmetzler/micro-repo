...
public class DocumentModelImpl implements DocumentModel, Cloneable {
    ...
    private static final Logger log = LoggerFactory.getLogger(DocumentModelImpl.class);
    ...
    public DocumentModelImpl(String parentPath, String name, String type, SchemaManager schemaManager,
        ...
        schemas = computeSchemas(getDocumentType(), instanceFacets, false, schemaManager);
    ...
    public DocumentModelImpl(String sid, String type, String id, Path path, DocumentRef docRef, DocumentRef parentRef,
        ...
        if (schemas == null) {
            this.schemas = computeSchemas(getDocumentType(), instanceFacets, isProxy, schemaManager);
        } else {
    ...
    protected DataModel loadDataModel(String schema) {
        ...
        if (!schemasOrig.contains(schema)) {
            ...
            DataModel dataModel = new DataModelImpl(schema, schemaManager);
        ...
        // load from session
        TypeProvider typeProvider = schemaManager;
    ...
    public boolean removeFacet(String facet) {
        ...
        schemas = computeSchemas(getDocumentType(), instanceFacets, isProxy(), schemaManager);
    ...
    private <T> T findAdapter(Class<T> itf) {
        // DocumentAdapterService svc =
        // Framework.getService(DocumentAdapterService.class);
        // if (svc != null) {
        ...
        // log.error("Document model cannot be adapted to " + itf + " because it has no
        // facet " + facet);
        // }
        // }
        // } else {
        // log.warn("DocumentAdapterService not available. Cannot get document model
        // adaptor for " + itf);
        // }
        return null;
    }
    ...
    public boolean followTransition(final String transition) {
        // TODO is it better to make public followTransition(DocumentRef, String,
    ...
    public void copyContent(DocumentModel sourceDoc) {
        ...
        for (String key : schemas) {
            ...
            if (oldDM != null) {
                ...
                Schema schema = schemaManager.getSchema(key);
                DocumentPartImpl part = new DocumentPartImpl.Builder().schema(schema)
                        .clearCompletePropertyBeforeSet(schemaManager.getClearComplexPropertyBeforeSet()).build();
                newDM = new DataModelImpl(part);
            }
    ...
    public static DataModel cloneDataModel(Schema schema, DataModel data, SchemaManager schemaManager) {
        DataModel dm = new DataModelImpl(schema.getName(), schemaManager);
    ...
    public DataModel cloneDataModel(DataModel data) {
        TypeProvider typeProvider = schemaManager;
        return cloneDataModel(typeProvider.getSchema(data.getSchema()), data, schemaManager);
    }
    ...
    public Property getProperty(String xpath) {
        ...
        String schemaName = getXPathSchemaName(cxpath, schemas, null, schemaManager);
        if (schemaName == null) {
            if (cxpath.indexOf(':') != -1) {
    ...
    public DocumentModel clone() throws CloneNotSupportedException {
        ...
        for (Map.Entry<String, DataModel> entry : dataModels.entrySet()) {
            ...
            DataModelImpl newData = new DataModelImpl(key, data.getMap(), schemaManager);
    ...
    /**
     * Sets the document id. May be useful when detaching from a repo and attaching
     * to another one or when unmarshalling a documentModel from a XML or JSON
     * representation
     *
     * @since 5.7.2
     */
