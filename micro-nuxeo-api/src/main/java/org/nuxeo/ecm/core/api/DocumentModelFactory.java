...
public class DocumentModelFactory {
    ...
    public static DocumentModelImpl createDocumentModel(Document doc, String sid, String[] schemas, SchemaManager sm,
        ...
        DocumentModelImpl docModel = new DocumentModelImpl(sid, type.getName(), doc.getUUID(), path, docRef, parentRef,
                null, facets, sourceId, repositoryName, doc.isProxy(), sm, css);
        ...
        if (schemas == null) {
            ...
            if (prefetchInfo != null) {
                schemas = prefetchInfo.getSchemas(sm);
            }
        }
        if (schemas != null) {
        ...
        for (String schemaName : loadSchemas) {
            ...
            docModel.addDataModel(createDataModel(doc, schema, sm));
        }
    ...
    public static DocumentModelImpl createDocumentModel(String sessionId, DocumentType docType, SchemaManager sm, CoreSessionProvider css) {
        ...
        for (Schema schema : docType.getSchemas()) {
            docModel.addDataModel(createDataModel(null, schema, sm));
        }
        return docModel;
    }
    ...
    public static DataModel createDataModel(Document doc, Schema schema, SchemaManager sm) {
        DocumentPart part = new DocumentPartImpl.Builder().schema(schema)
                                                          .clearCompletePropertyBeforeSet(
                                                                  sm.getClearComplexPropertyBeforeSet())
                                                          .build();
    ...
    public static DocumentModel writeDocumentModel(DocumentModel docModel, Document doc, SchemaManager sm,
        ...
        DocumentModel newModel = createDocumentModel(doc, docModel.getSessionId(), null, sm, css);
     ...
     */
    public static DocumentModelRefresh refreshDocumentModel(Document doc, int flags, String[] schemas, SchemaManager sm)
        ...
        if ((flags & DocumentModel.REFRESH_CONTENT) != 0) {
            ...
            for (int i = 0; i < schemas.length; i++) {
                @SuppressWarnings("deprecation")
                DocumentPart part = new DocumentPartImpl.Builder().schema(typeProvider.getSchema(schemas[i]))
                                                                  .clearCompletePropertyBeforeSet(
                                                                          sm.getClearComplexPropertyBeforeSet())
                                                                  .build();
    ...
    public static DocumentModel createDocumentModel(String type, String id, SchemaManager sm, CoreSessionProvider css) {
        ...
        docType.getSchemas().forEach(schema -> doc.addDataModel(createDataModel(null, schema, sm)));
        return doc;
    }
